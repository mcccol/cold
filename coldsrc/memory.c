...
#ifdef TDEBUG
    /* set up a little semaphore reflexive pointer */
    *(void **)p = p;
    p = ((void **)p) + 1;
#endif
...
void tfree(void *ptr, size_t size)
{
    ...
    t = (size - 1) / TRAY_INC;
#ifndef NDEBUG
    /* exhaustively test to ensure ptr isn't freed twice */
    for (tp = trays[t]; tp; tp = tp->next)
      assert(tp != ptr);
#endif
#ifdef TDEBUG
    assert(((Tlist *)ptr)->next == (Tlist *)ptr); /* check reflexive ptr for free-twice */
#endif
