...
String *string_new(int size_needed)
{
    ...
    cnew = (String *) emalloc(sizeof(String) + sizeof(char) * size);
    cnew->start = 0;
    cnew->len = 0;
    cnew->size = size;
    cnew->refs = 1;
...
void string_pack(String *str, FILE *fp)
{
  ...
  /*fprintf(stderr, "string_pack: %s len: %d @%d\n", str, str->len, ftell(fp));*/
}
...
String *string_unpack(FILE *fp)
{
    ...
    str->s[len] = 0;
    /*fprintf(stderr, "string_unpack: %s %d result: %d @%d\n", str, len, result, ftell(fp));*/
    return str;
}
...
{
  assert((str1->refs > 0) && (str2->refs > 0));
...
static String *prepare_to_modify(String *str, int start, int len)
{
	...
	while (size < len)
	    size = size * 2 + MALLOC_DELTA;
	str = (String *)erealloc(str, sizeof(String) + (size * sizeof(char)));
	str->size = size;
	return str;
    } else {
	if (str->reg) {
