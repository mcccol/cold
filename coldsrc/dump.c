...
/* Binary backup.  This dump must not allocate any memory, since we may be
 * performing it under low-memory conditions. */
int binary_backup(void)
{
    binary_dump();
    return system("cp -R binary binary.bak");
}
...
void text_dump_read(FILE *fp)
{
	...
	*q = 0;
	...
	if (!strnccmp(line->s, "parent", 6) && isspace(line->s[6])) {
	    ...
	    ident_discard(name);
	} else if (!strncmp(line->s, "debug", 5)) {
	  /* turn on/off assembler dump */
	  if (!strncmp(line->s, "debug off", 9)) {
	    debug = 0;
	  } else {
	    debug = 1;
	  }
