...
Dict *dict_new(List *keys, List *values)
{
    ...
    if (list_length(keys) != list_length(values)) {
      return (Dict *)0;
    }
...
int dict_order(Dict *dict1, Dict *dict2)
{
  return list_order(dict1->keys, dict2->keys) || list_order(dict1->values, dict2->values);
}
