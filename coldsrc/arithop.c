...
void op_negate(void)
{
    ...
    if (d->type != INTEGER) {
      type_error(d, integer_id, "Argument (%D) is not an integer.", d);
    } else {
	/* Replace d with -d. */
	d->u.val *= -1;
    }
...
void op_multiply(void)
{
    ...
    if (d1->type != INTEGER) {
        type_error(d1, integer_id, "Left side (%D) is not an integer.", d1);
    } else if (d2->type != INTEGER) {
        type_error(d2, integer_id, "Right side (%D) is not an integer.", d2);
    } else {
...
void op_divide(void)
{
    ...
    if (d1->type != INTEGER) {
        ...
        data_handler(d2, div_id, "Attempt to divide %D by zero.", d1);
    } else {
	/* Replace d1 with d1 / d2, and pop d2. */
	d1->u.val /= d2->u.val;
	pop(1);
...
void op_add(void)
{
    ...
    if (d1->type == INTEGER || d2->type == INTEGER) {
        if (d1->type != INTEGER) {
	  type_error(d1, integer_id, "%D must be integer.", d1);
        } else if (d2->type != INTEGER) {
	  type_error(d2, integer_id, "%D must be integer.", d2);
        } else {
	  /* Replace d1 with d1 + d2, and pop d2. */
	  d1->u.val += d2->u.val;
	  pop(1);
        ...
        if (d1->type != STRING) {
	  type_error(d1, string_id, "%D must be string.", d1);
        } else if (d2->type != STRING) {
	  type_error(d2, string_id, "%D must be string.", d2);
        } else {
	  ...
	  pop(1);
        ...
        if (d1->type != LIST) {
	  type_error(d1, list_id, "%D must be list.", d1);
        } else if (d2->type != LIST) {
	  type_error(d2, list_id, "%D must be list.", d2);
        } else {
	  anticipate_assignment();
	  ...
	  pop(1);
        ...
        data_obj_handler(d1, d1, type_id, "Cannot add %D and %D.", d1, d2);
	return;
    }
}
...
void op_splice_add(void)
{
    ...
    if (d1->type != LIST) {
        type_error(d1, list_id, "%D is not a list.", d1);
    } else if (d2->type != LIST) {
        type_error(d2, list_id, "%D is not a list.", d2);
    } else {
      anticipate_assignment();
      d1->u.list = list_append(d1->u.list, d2->u.list);
      pop(1);
...
void op_subtract(void)
{
    ...
    /* Make sure we're subtracting two integers. */
    if (d1->type != INTEGER) {
        type_error(d1, integer_id, "Left side (%D) is not an integer.", d1);
    } else if (d2->type != INTEGER) {
        type_error(d2, integer_id, "Right side (%D) is not an integer.", d2);
...
void op_in(void)
{
    ...
    if (d2->type == LIST) {
	pos = list_search(d2->u.list, d1, 0);
	pop(2);
	push_int(pos + 1);
	return;
    }
    ...
    if (d2->type != STRING) {
        type_error(d2, list_id, "Cannot search for %D in %D.", d1, d2);
	return;
    }

    if (d1->type != STRING) {
        type_error(d1, string_id, "Cannot search for %D in %D.", d1, d2);
	return;
    }
...
void op_bitand(void)
{
    ...
    if (!func_init_2(&args, INTEGER, INTEGER))
	return;

    args[0].u.val = args[0].u.val & args[1].u.val;
    pop(1);
}
